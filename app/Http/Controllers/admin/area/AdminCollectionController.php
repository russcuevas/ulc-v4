...
class AdminCollectionController extends Controller
{
    ...
    public function AdminEditPaymentCollection(Request $request, $paymentId)
    {
        $request->validate([
        ...
        if ($payment->is_collected == 1) {
            ...
            if ($loan) {
                $oldCollection = (float) ($payment->collection ?? 0);
                $newBalance = (float) $loan->balance + ($oldCollection - $newCollection);
                $newBalance = max($newBalance, 0);
    ...
    public function AdminSavePaymentCollection(Request $request)
    {
        $request->validate([
    ...
    public function AdminReversePaymentCollection(Request $request)
    {
        $request->validate([
            'loan_id' => 'required|exists:clients_loans,id',
            'payment_id' => 'nullable|exists:clients_payments,id',
    ...
    public function AdminWeeklyCollectionPage($location)
    {
        ...
        if ($areas->isEmpty()) {
            return redirect()->back()->with('error', 'Location not found or no areas under this location.');
        }

        // Group by areas_name to concatenate collectors on a single line
        $locationAreas = $areas->groupBy('areas_name')->map(function ($group) {
            $first = $group->first();
    ...
    public function AdminWeeklyCollectClientsPayment(Request $request, $location)
    {
        ...
        if ($allMatchedAreas->isEmpty()) {
            return response()->json(['message' => 'No areas found in this location.'], 404);
        }
        ...
        if ($loans->isEmpty()) {
            return response()->json(['message' => 'No active loans found in this location.'], 404);
        }
